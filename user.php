<?php 
	...
	if($data['user_level_id'] == 3) $heading = "Designer";
?> 
<script>
		...
		<div class="col1">
			<div class="contact">
				...
				<form action="lib/user.php" enctype="multipart/form-data" method="post" name="frm_user" onsubmit="return validateForm(this)">
					...
					<?php if(!$_SESSION['user_details']['user_id']) {?>
					<ul class="forms">
						<li class="txt">Register As</li>
						<li class="inputfield">
							<select name="user_level_id" id="user_level_id" class="bar" required/>
								<?php echo get_new_optionlist("role","role_id","role_name",$data[user_level_id],"role_show = 1"); ?>
							</select>
						</li>
					</ul>
					...
					<ul class="forms">
						<li class="txt">Profile Description</li>
						<li class="textfield"><textarea name="user_profile" cols="" rows="6" required><?=$data[user_profile]?></textarea></li>
					</ul>
					<?php } ?>
					<div class="clear"></div>
					<ul class="forms">
